...
return `<tr class="payee-row"> <td> <div style="display:flex;align-items:center;gap:10px"> <div class="payee-row-avatar" style="background:${avatarColor}18;border:1.5px solid ${avatarColor}40;color:${avatarColor}">${initials}</div> <div> <div style="font-weight:600;font-size:.875rem">${esc(p.name)}</div> ${p.notes ? `<div style="font-size:.72rem;color:var(--muted);margin-top:1px">${esc(p.notes)}</div>` : ‘’}
</div>
</div>
</td>
...
if(!cnt) return ‘’;
return `<div class="payee-summary-chip" onclick="scrollPayeeGroup('${g.key}')" style="border-left:3px solid ${g.color}"> <span>${g.icon}</span> <span style="font-weight:600;color:var(--text)">${g.label}</span> <span class="badge" style="background:${g.colorLt};color:${g.color};border:1px solid ${g.color}30">${cnt}</span> </div>`;
});
...
return `<div class="payee-group-wrap" id="payeeGroup-${g.key}"> <div class="payee-group-header" onclick="togglePayeeGroup('${g.key}')"> <div class="payee-group-icon" style="background:${g.colorLt}">${g.icon}</div> <span class="payee-group-title">${g.label}</span> <div class="payee-group-meta"> <span class="badge" style="background:${g.colorLt};color:${g.color};border:1px solid ${g.color}30;font-size:.75rem">${g.items.length} registro${g.items.length!==1?'s':''}</span> </div> <span class="payee-group-arrow${expanded?'':' collapsed'}">▼</span> </div> <div class="payee-group-body${expanded?'':' collapsed'}" id="payeeGroupBody-${g.key}"> <div class="table-wrap" style="margin:0"> <table style="border-radius:0"> <thead><tr><th>Nome</th><th>Categoria Padrão</th><th style="width:80px;text-align:center">Transações</th><th style="width:70px"></th></tr></thead> <tbody>${g.items.map(p=>payeeRow(p)).join('')}</tbody> </table> </div> </div> </div>`;
}).join(’’);
}
...
const sel=document.getElementById(‘payeeCategory’);
// Hierárquico: pais primeiro, filhos indentados, agrupados por tipo
const buildCatOptions = () => {
const cats = state.categories;
const parents = cats.filter(c => !c.parent_id).sort((a,b) => {
// tipo primeiro (despesa antes de receita), depois nome
if (a.type !== b.type) return a.type === ‘despesa’ ? -1 : 1;
...
opts += `<optgroup label="${p.type === 'despesa' ? '💸 Despesas' : '💰 Receitas'}">`;
lastType = p.type;
}
opts += `<option value="${p.id}">${p.icon ? p.icon + ' ' : ''}${esc(p.name)}</option>`;
const children = cats.filter(c => c.parent_id === p.id).sort((a,b) => a.name.localeCompare(b.name));
for (const c of children) {
opts += `<option value="${c.id}">&nbsp;&nbsp;↳ ${c.icon ? c.icon + ' ' : ''}${esc(c.name)}</option>`;
}
}
if (lastType !== null) opts += ‘</optgroup>’;
return opts;
};
sel.innerHTML = buildCatOptions();
sel.value=form.default_category_id||’’;
...
body.innerHTML = items.map((item, idx) => ` <tr style="border-bottom:1px solid var(--border);${item.exists?'opacity:.55':''}"> <td style="padding:6px 12px;color:var(--text)">${esc(item.name)}</td> <td style="padding:6px 8px;text-align:center"> ${item.exists ? '<span style="font-size:.72rem;font-weight:600;color:var(--muted);background:var(--bg3);padding:2px 7px;border-radius:20px">Existente</span>' : '<span style="font-size:.72rem;font-weight:600;color:var(--green);background:var(--green-lt);padding:2px 7px;border-radius:20px">Novo</span>' } </td> <td style="padding:6px 8px;text-align:center"> <input type="checkbox" ${item.selected?'checked':''} onchange="payeeClipboardToggleItem(${idx},this.checked)"> </td> </tr>`).join(’’);
